...
function getObject(page){
	...
	$.getJSON(baseUrl+"menuinfo/getObjectById?id="+id, function(fbean){
		...
		$("#index_icon").attr("onclick","back('template5.html?id="+id+"')");
			...
			if(data.state){
			  ...
			  if(list.length>0){
				       for(var index in list){
				    	   ...
				    	   str+='<a class="ad-item" href="javascript:void(0);"  onclick="setContent('+bean.id+')">'+
					            '<div class="time-box">'+
					            '<img src="'+bean.img+'">'+
					            '</div>'+
					            '<div class="ad-text">'+
					            '<div class="title2">'+bean.title+'</div>'+
					            '<p class="overflow-2">'+bean.content+'</p>'+
					            '</div>'+
					            '<div class="tips">'+
...
function setContent(id){
    ...
    $.getJSON(baseUrl+"newsinfo/getObjectById?id="+id, function(bean){
    	cont+='<div class="text-box deteil-box"><div class="text-title">'+bean.title+'</div>'+bean.content+'</div>';	  
        $("#temp_cont").html(cont);
    });     
    $("#paginationid").hide();
