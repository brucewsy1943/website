...
function getObject(page){
	...
	$.getJSON(baseUrl+"menuinfo/getObjectById?id="+id, function(fbean){
		...
		$("#index_icon").attr("onclick","back('template2.html?id="+id+"')");
		
		$.getJSON("../../newsinfo/getObjectListIsFb?page="+page+"&rows="+rows+"&type="+type, function(data){
			if(data.state){
			  ...
			  if(list.length>0){
				       for(var index in list){
				    	   let bean=list[index];
				    	   str+='<a class="ad-item" href="'+bean.intro+'" target="_blank">'+
					            '<div class="time-box">'+
					            '<img src="'+bean.img+'">'+
					            '</div>'+
					            '<div class="ad-text">'+
