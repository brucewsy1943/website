...
function getObject(page){
	...
	$.getJSON(baseUrl+"menuinfo/getObjectById?id="+id, function(fbean){
		...
		$("#index_icon").attr("onclick","back('template9.html?id="+id+"')");
		...
		$.getJSON(baseUrl+"newsinfo/getObjectListIsFb?page="+page+"&rows="+rows+"&type="+type, function(data){
			if(data.state){
			  ...
			  if(list.length>0){
				  ...
				  str+='<li>'+
			              '<span class="span1">职位名称</span>'+
			              '<span class="span1">工作地点</span>'+
			              '<span class="span2">招聘人数</span>'+
				      ...
				      str+='<li>'+
				    	     '<a href="javascript:void(0);"  onclick="setContent('+bean.id+')">'+
				                '<span class="span1">'+bean.title+'</span>'+
				                '<span class="span1">'+bean.intro+'</span>'+
				                '<span class="span2">'+bean.subhead+'</span>'+
				                '<span class="span2">'+bean.fbDate+'</span>'+
				              '</a>'+
				            '</li>';
				       }
...
function setContent(id){
    ...
    $.getJSON(baseUrl+"newsinfo/getObjectById?id="+id, function(bean){
    	cont+=' <div class="white-panel recruit-box job-box">'+
                 '<div class="clearfix">'+
                    '<div class="left">'+
                       '<div class="brisc-title no-border" id="job_name">'+bean.title+'</div>'+
                       '<div class="job-name" ><b>招聘人数：</b></div>'+
                       '<div id="job_sum">'+bean.subhead+'人</div><br/>'+bean.content+'</div>'+
                '</div>'+
              '</div>';	  
        $("#temp_cont").hide();
        $("#content_text").show();
